...
def split_heads(tensors, bsz, seqlen, n_head):
    assert isinstance(tensors, (tuple, list))
    return [x.view(bsz, seqlen, n_head, -1).transpose(1, 2) for x in tensors]
...
class MultiScaleRetention(nn.Module):
    ...
    def __init__(self, config: RetNetConfig):
        ...
        self.gn = nn.GroupNorm(num_groups=config.n_head, num_channels=config.v_dim, affine=False)
        self.xpos = XPOS(config.qk_dim)

        # initialize gamma
        if config.use_default_gamma:
            gamma = 1 - 2**(-5 - torch.arange(0, config.n_head, dtype=torch.float))
        else:
            s = torch.log(torch.tensor(1 / 32))
            e = torch.log(torch.tensor(1 / 512))
            gamma = 1 - torch.exp(torch.linspace(s, e, config.n_head))  # [h,]
    ...
    def get_parallel_decay_mask(self, length, retention_mask=None):
        ...
        range_tensor = range_tensor[None, :, None].expand(self.config.n_head, length, 1)
    ...
    def get_recurrent_decay(self):
        decay = self.decay.view(1, self.config.n_head, 1, 1)
        return decay
    ...
    def get_chunkwise_decay(self, chunk_size, retention_mask=None):
        ...
        # decay of the chunk
        chunk_decay = self.decay.view(1, self.config.n_head, 1, 1)**chunk_size
        # cross-chunk decay
        exponent = torch.arange(chunk_size, dtype=torch.float,
                                device=decay_mask.device).unsqueeze(0) + 1
        inner_decay = (self.decay.unsqueeze(-1)**exponent).view(1, self.config.n_head,
                                                                chunk_size, 1)
    ...
    def forward(
        ...
        q, k, v = split_heads((q, k, v), B, T, self.config.n_head)
        # retention
...
class RetNetBlock(nn.Module):
    ...
    def __init__(self, config: RetNetConfig):
        ...
        self.ffn = nn.Sequential(
            ...
            # nn.Linear(config.hidden_size, bias=config.use_bias_in_mlp),
            nn.GELU(),
